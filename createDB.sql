...
CREATE TABLE  job_title (title_id INT AUTO_INCREMENT, title VARCHAR(20) NOT NULL, CONSTRAINT job_title__pk PRIMARY KEY (title_id));

CREATE TABLE employee (employee_id INT AUTO_INCREMENT, personal_data_id INT, title_id INT, company_id INT, CONSTRAINT employee_pk PRIMARY KEY (employee_id), CONSTRAINT emp_company_fk FOREIGN KEY (company_id) REFERENCES company (company_id), CONSTRAINT `emp_pd_fk` FOREIGN KEY (`personal_data_id`) REFERENCES `personal_data` (`personal_data_id`));
