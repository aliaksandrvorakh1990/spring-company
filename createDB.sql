...
CREATE TABLE employee (employee_id INT AUTO_INCREMENT, personal_data_id INT UNIQUE, title_id INT, company_id INT, CONSTRAINT employee_pk PRIMARY KEY (employee_id), CONSTRAINT emp_company_fk FOREIGN KEY (company_id) REFERENCES company (company_id), CONSTRAINT `emp_pd_fk` FOREIGN KEY (`personal_data_id`) REFERENCES `personal_data` (`personal_data_id`), CONSTRAINT `emp_jobst_fk` FOREIGN KEY (`title_id`) REFERENCES `job_title` (`title_id`));
