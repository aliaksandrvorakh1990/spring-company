...
public class CompanyController {
    ...
    @ApiOperation(value = "Get a list of existing companies from the database",
            response = CompanyViewModel.class, responseContainer = "List")
    @ApiResponses({ 
    ...
    @ApiResponses({ 
        @ApiResponse(code = 500, message = "The company does not exist " + 
                "or Problems with server")
    })
    @GetMapping("/companies/{id}")
    public CompanyViewModel getCompany(
            @ApiParam(value = "The company will be gotten from the database " + 
    ...
    @ApiResponses({
        ...
        @ApiResponse(code = 500, message = "The company was not created " + 
                "in the database or Problems with server"),
    })
    @PostMapping("/companies")
    public IdViewModel createCompany(
            @ApiParam(value = "The company data for creating in the database.", 
                    required = true)
    ...
    @ApiResponses({
        @ApiResponse(code = 400, message = "Bad Request: wrong data"),
        @ApiResponse(code = 500, message = "The company was not updated " + 
                "in the database or Problems with server")
    })
    @PutMapping("/companies/")
    public void updateCompany(
            @ApiParam(value = "The company data for updating in the database.", 
                    required = true)
    ...
    @ApiOperation(value = "Delete an existing company by Id from the database.", 
            notes = "ID has to be greater than zero.")
    @ApiResponses({ 
        @ApiResponse(code = 500, message = "The company was not delated " + 
                "from the database or Problems with server")
    })
    @DeleteMapping("/companies/{id}")
    public void deleteCompany(
            @ApiParam(value = "The company will be deleted from the database " + 
                    "by his ID.", required = true)
            @PathVariable("id") @Positive @NotNull Integer id) {
        companyService.delete(id);
    }
    
    @ApiOperation(value = "Create and read a random employee " + 
            "from external source.", notes = "ID has to be greater than zero.", 
            response = EmployeeViewModel.class)
    @ApiResponses({
        @ApiResponse(code = 500, message = "The company does not exist or " + 
    ...
    public  DeferredResult<EmployeeViewModel> getRandomEmployee(
      	...
      	Observable<EmployeeViewModel> employeeObservable = companyService
                .randomEmployee(id);
      	DeferredResult<EmployeeViewModel> deferredResult = 
                new DeferredResult<>();
      	
      	employeeObservable.subscribe(deferredResult::setResult, 
